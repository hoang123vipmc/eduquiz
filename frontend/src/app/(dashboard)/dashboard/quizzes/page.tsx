  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-white mb-2">Đề thi của tôi</h2>
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-[#4F7CFF] hover:bg-[#6D91FF] text-white px-6 py-3 rounded-xl font-semibold shadow-[0_4px_12px_rgba(79,124,255,0.3)] transition-all hover:shadow-[0_6px_16px_rgba(79,124,255,0.4)] hover:-translate-y-0.5 active:translate-y-0"
        >
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="animate-pulse flex flex-col bg-[#0f172a] rounded-[20px] overflow-hidden border border-white/5 h-[340px]">
              <div className="h-32 bg-[#18233b]"></div>
              <div className="p-6 flex-1 flex flex-col gap-4">
                <div className="h-6 bg-[#18233b] rounded-md w-3/4"></div>
                <div className="h-4 bg-[#18233b] rounded-md w-full"></div>
                <div className="h-4 bg-[#18233b] rounded-md w-2/3"></div>
                <div className="mt-auto h-12 bg-[#18233b] rounded-xl w-full"></div>
              </div>
            </div>
          ))}
        </div>
      ) : quizzes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-[#0f172a] rounded-[20px] border border-white/5">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">Chưa có đề thi nào</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quizzes.map((quiz) => (
            <div key={quiz.id} className="group flex flex-col bg-[#0f172a] rounded-[20px] overflow-hidden border border-white/5 hover:border-white/10 hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-all duration-300">
              
              {/* TOP: Banner */}
              <div className="h-32 relative bg-gradient-to-br from-[#1e293b] to-[#0f172a] flex items-end p-4 border-b border-white/5">
                {/* Abstract shape */}
                ...
                <button 
                  ...
                  className="absolute top-4 right-4 z-20 p-2 bg-[#EF4444]/20 text-red-200 hover:bg-[#EF4444] hover:text-white rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100"
                  title="Xóa đề thi"
                >
              ...
              <div className="p-6 flex-1 flex flex-col">
                <h3 className="text-lg font-bold text-white line-clamp-1 mb-2 group-hover:text-[#4F7CFF] transition-colors" title={quiz.title}>
                  {quiz.title}
                </h3>
                <p className="text-[14px] text-slate-400 line-clamp-2 mb-6 flex-1 leading-relaxed">
                  {quiz.description || "Chưa có mô tả cho đề thi này."}
                </p>

                {/* BOTTOM: Stats */}
                <div className="flex items-center justify-between text-[13px] text-slate-400 font-medium mb-6">
