  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#4F7CFF] to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-blue-500/20">
          <History className="w-7 h-7" />
        </div>
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-white mb-1">Lịch sử làm bài</h2>
      ...
      {loading ? (
        <div className="space-y-4">
          <div className="h-[60px] bg-[#18233b] rounded-xl animate-pulse border border-white/5"></div>
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-[72px] bg-[#0f172a] rounded-xl animate-pulse border border-white/5"></div>
          ))}
        </div>
      ) : history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-[#0f172a] rounded-[20px] border border-white/5 shadow-2xl">
          <div className="w-24 h-24 bg-[#18233b] rounded-full flex items-center justify-center mb-6">
            <History className="w-12 h-12 text-slate-500" />
          </div>
          <h3 className="text-2xl font-bold text-white mb-3">Chưa có lịch sử</h3>
        ...
        <div className="bg-[#0f172a] rounded-[20px] border border-white/5 shadow-[0_8px_30px_rgba(0,0,0,0.4)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-[15px] text-left border-collapse">
              <thead className="bg-[#071026] text-slate-400 font-semibold border-b border-white/5">
                <tr>
              ...
              <tbody className="divide-y divide-white/5">
                {history.map((h) => {
                  ...
                  return (
                    <tr key={h.id} className="hover:bg-[#18233b] transition-colors group">
                      <td className="px-6 py-5">
                        <div className="font-semibold text-slate-200 max-w-[250px] truncate group-hover:text-white transition-colors" title={h.quiz?.title}>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-300 font-medium">
                            {h.correct_answers} <span className="text-slate-500 text-xs">/ {h.quiz?.total_questions || '-'}</span>
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-5 text-right">
                        <button 
                          ...
                          className="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white font-medium transition-colors"
                        >
